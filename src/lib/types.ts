...
export type YutGame = {
  ...
  rolls: Throw[];
  /**
   * **남은 던질 횟수.** 차례가 오면 1, 던지면 1 줄고, 윷 · 모가 나오거나 잡으면 1 는다.
   *
   * 9/23 까지는 `pending` 참/거짓 하나였는데, 그러면 **빚진 던지기가 둘이어도 한 번으로 뭉개졌다** —
   * 윷을 던지고(빚 1) 그 윷으로 잡으면(빚 2) 다음 던지기가 `pending = isExtra(값)` 으로
   * 덮어써져서 한 번이 날아갔다. 그래서 숫자로 바꿨다.
   */
  owe: number;
  /** 옛 문서 호환 — `owe` 가 없으면 이걸 1/0 으로 읽는다. 이벤트가 끝나면 같이 지운다 */
  pending?: boolean;
  /**
